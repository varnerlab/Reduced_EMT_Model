...
function main(readout_index,perturbed_index_array,delta_array) # what we want to look at, how we want to perturb, difference
...
function calculate_auc_base(parameter_array,readout_index)
  ...
  # will need to rerun to steady state and load a blank/default data dictionary
  # need to initialize each model using a parameter array from POETs
...
function calculate_auc_perturbed(parameter_array,readout_index,perturbed_index_array,delta_array)
  # perturbed_index_array -- here we are selecting a species initial condition to perturb but I will
  # need to send the simulation files
  # Need to do calculate the AOC from the simulations twice
  # Because we need both ecadherin robustness and vimentin robustness
  # Sample 100 sets (mod 100)
  # at end just plot a satter (log)
  # keep in mind: definition of robustness - can cut off the smalls or run it with a 1
